...
$DiskSettings = [PSCustomObject]@{
    ...
    MinimumSize          = $ENV:MinimumDiskSize
}
####### Disk settings
try {
...
$DisksOutOfSpace = Foreach ($Volume in $Volumes) {
    ...
    if ($volume.SizeRemaining -lt $absolute -or ([Math]::Round(($volume.SizeRemaining / $volume.Size * 100), 0)) -lt $percent -and $volume.size -gt $DiskSettings.MinimumSize) {
        [PSCustomObject]@{
